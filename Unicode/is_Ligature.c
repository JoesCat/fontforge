...
const uint8 ____ligature_ind[] = {
  ...
  0x00, 0x01, 0x80, 0x80, 0x80, 0x80, 0x80, 0x83,
  0x83, 0x83, 0x83, 0x83, 0x83, 0x83, 0x83, 0x80};

/* MISC_verbose: here done index=511_512, have tis=2 2, til=0 0 */
const uint16 ____ligature_altd16[] = {
  ...
  /* Ufdfb */	0x062c, 0x0644, 0x0020, 0x062c, 0x0644, 0x0627, 0x0644, 0x0647};

const uint8 ____ligature_altis[] = {
  0x00, 0x12, 0x1a};

int Ligature_get_altC(int n) {
    int i,j;
    if ( n<0 || n>=512 )
	return( -1 );
    if ( (i=____ligature_ind[n])&0x80 ) {
	j = i & 0x7f;
	for ( i=0; j; j=j>>1 )
	    if ( j&1 ) ++i;
    } else
	int32 Ligature_get_altA(int n,int a) {
    int i,j;
    const unichar_t *upt;
    int32 u;
    if ( n<0 || n>=512 || a<0 || a>=Ligature_get_altC(n) )
	return( -1 );
    const uint8 ____vulgfrac_ind[] = {
  0x87, 0x87, 0x87, 0x87, 0x87, 0x8f, 0x87, 0x87,
  0x87, 0x87, 0x87, 0x87, 0x87, 0x87, 0x87, 0x87,
  0x87, 0x87, 0x87};

/* MISC_verbose: here done index=19_19, have tis=0 0, til=0 0 */
int VulgFrac_get_altC(int n) {
    int i,j;
    if ( n<0 || n>=19 )
	return( -1 );
    j = ____vulgfrac_ind[n] & 0x7f;
    for ( i=0; j; j=j>>1 )
	if ( j&1 ) ++i;
    return( i );
}

int32 VulgFrac_get_altA(int n,int a) {
    int i,j;
    const unichar_t *upt;
    int32 u;
    if ( n<0 || n>=19 || a<0 || a>=VulgFrac_get_altC(n) )
	return( -1 );
    i = ____vulgfrac_ind[n];
    j = i & 0x7f;
    for ( i=0; a; ++i )
	if ( j&(1<<i) ) --a;
    u = VulgFrac_get_U(n);
...
/* MISC_verbose: here done index=34_50, have tis=0 0, til=0 0 */
int Fraction_get_altC(int n) {
    int i,j;
    if ( n<0 || n>=50 )
	return( -1 );
    j = ____fraction_ind[n] & 0x7f;
    for ( i=0; j; j=j>>1 )
	if ( j&1 ) ++i;
    return( i );
}

int32 Fraction_get_altA(int n,int a) {
    int i,j;
    const unichar_t *upt;
    int32 u;
    if ( n<0 || n>=50 || a<0 || a>=Fraction_get_altC(n) )
	return( -1 );
    i = ____fraction_ind[n];
    j = i & 0x7f;
    for ( i=0; a; ++i )
	if ( j&(1<<i) ) --a;
    u = Fraction_get_U(n);
